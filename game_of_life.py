...
class Board(object):
    ...
    def seed(self, block_coords):
        ...
        for x, y in block_coords:
            self.block_list[(x, y)].set_live(self.canvas)
    ...
    def get_block_neighbors(self, block):
        ...
        for i in range(max(x-1, 0), min(x+2, width)):
            for j in range(max(y-1, 0), min(y+2, height)):
                if i != x or j != y:
                    neighbors.append(self.block_list[(i, j)])
    ...
    def simulate(self):
        ...
        #### YOUR CODE HERE #####
        for block in self.block_list.values():
            noLiveNeighbors = sum([nb.is_live() for nb in self.get_block_neighbors(block)])
            if block.is_live():
                if noLiveNeighbors < 2 or noLiveNeighbors > 3:
                    block.new_status = 'dead'
            else:
                if noLiveNeighbors == 3:
